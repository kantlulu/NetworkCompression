...
class FiltersPruner(object):
    ...
    def _get_prune_indices(self, name, module, prune_rate, mode='filter-a'):
        ...
        elif 'filter-nggm' in mode:
            f_scores = (
                self.gamma * min_max_scalar(get_gm_dists(f_w)) +
                (1 - self.gamma) * min_max_scalar(get_l1_scores(f_g))
            )
    ...
    def _set_use_grad(self, mode='filter-r'):
        self.use_grad = False
        if mode in ['filter-nggm']:
            self.use_grad = True
